...
# USER STATS
    ...
    def create_user(self, userid, username):
        ...
        if i != -1:
            ...
            if not self.has_key(self.config["users"][i], "unlocked_delivery"):
                self.config["users"][i]["unlocked_delivery"] = False
            return i


        new_user = {"userid": str(userid),
                    "username": username,
                    "unlocked_delivery": False,
                    "downloads": 0,
                    "priority": 1,
                    "latest_item_time": 0,
                    "downloaded_from": []}
        ...
        while self.running:
            ...
            try:
                ...
                if not "few minutes" in str(e):
                    self.queue.remove(item)
                    #self.api.sendMessage(str(item["userid"]), "Resend that video please.")
                #self.errors += 1
                self.reload_api()
                self.sleep = [30, 120]
            time.sleep(1)
...
class InboxHandler(object):
    def __init__(self, API, config, uploader, d_uploader):
        ...
        #self.d_uploader_list = d_uploader
        #self.d_uploader = self.d_uploader_list[0]
    ...
    def is_multipost_expected(self, userid):
        #MULTIPATH + str(item.userid) + ".json"
    ...
    def get_d_uploader(self):
        upl = self.d_uploader_list[0]
        for u in self.d_uploader_list:
            if len(upl.queue) > len(u.queue):
                upl = u
    ...
    def is_post_queued(self, media_id, username):
        ...
        for upl in self.uploader_list:
            if upl.queue_contains_post(media_id, username):
                return True
        #for upl in self.d_uploader_list:
        #    if upl.queue_contains_post(media_id, username):
        #        return True
        return False
...
#item handler
    def handle_video(self, username, item, same_queue=False, videojson = None, bypass = False):
        ...
        same_queue = False
        #if self.queue_total() >= 13 and user["unlocked_delivery"]:
        #    if not same_queue:
        #        uploader = self.get_d_uploader()
        #        self.d_uploader = uploader
        #    else:
        #        uploader = self.d_uploader
        #    self.api.sendMessage(str(item.author_id), "That video will be delivered by @instaadelivery")
        #else:
        if not same_queue:
    ...
    def handle_link(self, username, item):
        ...
        if self.cfg.get_user(item.author_id)["latest_item_time"] == item.timestamp:
            return
        self.cfg.user_set_itemtime(item.author_id, username, item.timestamp)
        raw_url = item.item["link"]["text"]
        if not "reel" in raw_url:
            return
        if "?igshid" in raw_url:
            raw_url = raw_url.split("?igshid")[0] + "?__a=1"
        cookies = self.api.get_cookie_string(raw=True)
        response = requests.get(raw_url, cookies=cookies)
        if response.status_code == 200:
            try:
                content = json.loads(response.content)
                url = content["graphql"]["shortcode_media"]["video_url"]
                ...
                uploader.send_media(url, item.item["item_id"], 2, str(item.author_id), str(item.author_id), username,
                                    item.get_item_poster(), item.timestamp, cut=False)
                logging.info("Added {u} to queue".format(u=username))
            except Exception as e:
                print(e)
    ...
    def do_delay_ad(self, username, item):
        ...
        if delay > 300:
            ...
            if uprankdelay > 150:
                return
            self.api.sendMessage(str(item.author_id), "There are {q} people in the queue therefore receiving your post takes more than 5 minutes. Please dm @bungbloopity if you would like to purchase priority queue!".format(q=self.queue_total()))
...
cfg = Config(Path("config_kowalski.json"))
...
cfg.add_text("requested", "I'm not following this account yet, please DM @bungbloopity if you would like the account to be followed")
cfg.add_text("images_not_supported", "Images are not supported yet.")
cfg.add_text("video_to_long", "This video is longer than 60s therefore we're unable to send it via dms.")
cfg.add_text("links_not_supported", "Links are currently not supported. If its a video resend it using the arrow button.")
cfg.add_text("blocked", "The post you were sending is either a post from someone who blocked us or an IGTV post. IGTV is not supported.")
cfg.add_text("deleted", "That account/post is currently shadowbanned. Try again later.")

cfg.add_text("welcome_0", "")
cfg.add_text("welcome_1", "")
#print(cfg.get_text("requested"))


mainlogin = InstagramLogin(username, password, Path("./sessions"))
api = mainlogin.api

#deliverylogin = InstagramLogin("instaadelivery", "1gP3G}]VxW_CV0+2", Path("./sessions"))
#d_api = deliverylogin.api

if not api.isLoggedIn:
    logging.error("Failed to login")
    exit()

#if not d_api.isLoggedIn:
#    logging.error("Failed to login deliverant")
#    exit()

uploaders = []
for x in range(0, 1):
    uploaderpath = Path("sessions/uploader_{0}.session".format(x))
...
#delivery_uploaders = []
#username = "instaadelivery"
#password = "1gP3G}]VxW_CV0+2"
#for x in range(0, 8):
#    uploaderpath = Path("sessions/deliveryuploader_{0}.session".format(x))
#    queuepath = Path("delivery_uploader{0}_queue".format(x))
...
#    test_upl.start()
#    print("delivery user" + uapi.username)
#    delivery_uploaders.append(test_upl)


inbox = InboxHandler(api, cfg, uploaders, [])
#dinbox = dInboxHandler(deliverylogin, cfg)
#dinbox.start()
inbox.run()

#cfg.create_user(1, "uwuser")
#cfg.create_user(2, "owoser")
#cfg.create_user(3, "eweser")
#cfg.save_config()
#upl.send_video("txt", 1)
#upl.send_video("txt", 2)
#upl.send_video("txt", 3)
